...
async function updateCars(id: number, model: string, licensePlate: string, year: number, color: string) {
  const cars = await carRepository.upsertCar(id, model, licensePlate, year, color);
  return cars;
}
